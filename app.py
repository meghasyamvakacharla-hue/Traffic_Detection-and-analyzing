...
if uploaded_file:
    ...
    if cap.isOpened():
        ...
        if ret:
            ...
            if st.sidebar.button("✨ Auto-Detect Settings"):
                with st.spinner("Analyzing traffic pattern..."):
                    try:
                        # 1. Try standard detection (YOLO)
                        ...
                        # 2. Fallback to color detection if YOLO found nothing (likely synthetic video)
                        if not detected_roi:
                        ...
                        if detected_roi and detected_line:
                            ...
                            st.session_state['auto_roi'] = detected_roi
                            st.session_state['auto_line'] = detected_line
                            st.success("Settings auto-detected!")
                            st.rerun()
                        else:
                            st.warning("Could not detect enough traffic to configure automatically.")
                    except Exception as e:
                        st.error(f"Auto-detection failed: {e}")

            # Defaults or Auto-detected values
            default_roi = [(int(width*0.3), int(height*0.4)), (int(width*0.7), int(height*0.4)), 
                          (int(width*0.7), int(height*0.6)), (int(width*0.3), int(height*0.6))]
            default_line = [(int(width*0.2), int(height*0.8)), (int(width*0.8), int(height*0.8))]
            
            if 'auto_roi' in st.session_state:
                roi_to_use = st.session_state['auto_roi']
            else:
                roi_to_use = default_roi
                
            if 'auto_line' in st.session_state:
                line_to_use = st.session_state['auto_line']
            else:
                line_to_use = default_line

            # Queue ROI Sliders
            with st.sidebar.expander("Queue Detection Area", expanded=True):
                q_x1 = st.slider("Queue X1", 0, width, roi_to_use[0][0])
                q_y1 = st.slider("Queue Y1", 0, height, roi_to_use[0][1])
                q_x2 = st.slider("Queue X2", 0, width, roi_to_use[2][0])
                q_y2 = st.slider("Queue Y2", 0, height, roi_to_use[2][1])
                queue_roi = [(q_x1, q_y1), (q_x2, q_y1), (q_x2, q_y2), (q_x1, q_y2)]
            
            # Stop Line Sliders
            with st.sidebar.expander("Stop Line", expanded=True):
                l_y = st.slider("Stop Line Y Position", 0, height, line_to_use[0][1])
                l_x1 = st.slider("Stop Line X1", 0, width, line_to_use[0][0])
                l_x2 = st.slider("Stop Line X2", 0, width, line_to_use[1][0])
