...
class TrafficAnalyzer:
    ...
    def analyze_traffic_pattern(self, video_path, num_samples=50):
        """
        Analyzes the video to automatically detect the traffic flow area (ROI) and stop line.
        Returns:
            roi_points: List of (x,y) tuples for the Queue ROI
            stop_line: List of two (x,y) tuples for the Stop Line
        """
        ...
        for i in range(0, total_frames, step):
            ...
            if self.use_color_detection:
                detections = self.detect_colored_rectangles(frame)
                for det in detections:
                    all_centroids.append(det['center'])
            else:
                # Use YOLO but lighter config if possible or just standard
                # We only need detections, no tracking
                ...
                if results[0].boxes is not None:
                    ...
                    for box, class_id in zip(boxes, class_ids):
                        if class_id in [2, 3, 5, 7]: # vehicles
                            x, y, w, h = box
                            all_centroids.append((float(x), float(y)))
        ...
        # Calculate bounding box of all centroids with some padding
        all_centroids = np.array(all_centroids)
        min_x = max(0, int(np.min(all_centroids[:, 0])) - 50)
        max_x = min(width, int(np.max(all_centroids[:, 0])) + 50)
        min_y = max(0, int(np.min(all_centroids[:, 1])) - 50)
        max_y = min(height, int(np.max(all_centroids[:, 1])) + 50)
        
        # Define ROI as the bounding box of activity
        # We might want to restrict it to a certain vertical range for queue detection
        # e.g., the "incoming" side. For now, use the full activity width and partial height.
        
        # Heuristic: Queue area is usually the bottom 60% of the activity area 
        # (assuming camera faces oncoming traffic or traffic moves away)
        # Let's assume standard CCTV View: Traffic flows towards bottom or top.
        # Safest is to just cover the activity area.
        
        roi_points = [
            (min_x, int(min_y + (max_y - min_y)*0.2)), # Start 20% down to avoid far background
            (max_x, int(min_y + (max_y - min_y)*0.2)),
            (max_x, max_y),
            (min_x, max_y)
        ]
        
        # Heuristic: Stop line is usually near the intersection/exit.
        # Let's place it at 80% of the activity height.
        stop_line_y = int(min_y + (max_y - min_y) * 0.8)
        stop_line = [(min_x, stop_line_y), (max_x, stop_line_y)]
        
        return roi_points, stop_line
